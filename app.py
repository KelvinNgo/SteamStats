...
def get_steam_userinfo(steam_id):
    ...
    return render_template("base.html", rv=rv['response']['players']['player'][0])
...
def new_user(resp):
    ...
    check = _steam_id_re.findall(resp.identity_url)[0]
    options = {
        'key': app.secret_key,
        'steamids': check #steam_id
    }
    url = 'http://api.steampowered.com/ISteamUser/' \
          'GetPlayerSummaries/v0001/?%s' % urllib.urlencode(options)
    rv = json.load(urlopen(url))
    ...
    return render_template("base.html", id=rv['response']['players']['player'][0])
